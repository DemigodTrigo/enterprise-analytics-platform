...
@RestController
@RequestMapping("/api/master/statuses")
public class StatusController {

    private final MasterCacheService cacheService;

    public StatusController(MasterCacheService cacheService) {
        this.cacheService = cacheService;
    }

    @PreAuthorize("hasAnyRole('ADMIN','USER')")
    @GetMapping
    public EAJson<List<StatusEntity>> getStatuses() {
        return EAJson.success(cacheService.getStatusesFromCache(),"Statuses loaded successfully");
    }
}